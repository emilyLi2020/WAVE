...
Every narration string shown in the session UI is generated by Gemma 4 E2B running inside the browser tab — base model + the appropriate LoRA for that phase (see `docs/models.md`).
...
- **Stretch LoRAs** (`lora-notification`, `lora-insights`) — MVP ships LoRAs 1–6 (medication ack, body scan, three wave phases, reflection) per `docs/models.md`. Notification and insights LoRAs train through the same process in `docs/model-training.md` when time allows.
