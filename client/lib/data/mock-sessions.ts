 ...
 * dashboard, history, and insights surfaces. Numbers are tuned so the
 * stats, weekday/hour heatmap, history list, and "this week" card all
 * stay internally consistent — and so the regenerate flow on
 * /insights gets a realistic input to reason over.
 *
 * Sessions are authored in chronological order through a compact
 * `SessionInput` shape and built into full `Session` records (with
 * auto-generated `s_NNN` ids and ISO timestamps). `MOCK_SESSIONS`
 * exposes them in newest-first order to match the conventions of the
 * history page.
 *
 * Patterns intentionally encoded in the data:
 *   - Thursday 5–7 PM is the densest weekday/hour cell (post-work
 *     stress cluster) and stays the visual hot-spot on the heatmap.
 *   - Friday/Saturday late-night entries skew social trigger and
 *     sometimes late or missed med status.
 *   - Sunday/Monday mornings skew physical trigger with missed-dose
 *     context (early-morning withdrawal pattern).
 *   - Med-on-time sessions show ~5 pt drops on average; missed-dose
 *     sessions show ~3 pt drops; late-dose sits between them.
 *   - Every weekday/hour cell has at least one low-intensity entry
 *     except Mon and Tue 5–8 AM (clinically implausible early-morning
 *     workday slots), so the dashboard heatmap reads as continuous
 *     coverage with a clear Thursday peak.
 *
 * Keep this module pure and SSR-safe: derivations use the ISO string
 * components directly so the values never depend on the runtime
 * timezone.
 */
...
function buildSession(input: SessionInput, idx: number): Session {
  ...
  // Synthesize an `endedAt` ~10 minutes after `startedAt` so the
  // history page or any future duration UI has a realistic value.
...
export const MOCK_WEEK_SUMMARY = {
  ...
  // Per-day adherence isn't represented in the Session type; the value
  // below is a static stand-in for the dashboard copy.
