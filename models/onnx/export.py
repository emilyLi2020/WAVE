...
def _cast_fp32_to_fp16(src: Path, dst: Path) -> None:
    ...
    streaming caster (cast_fp16.py) in a subprocess: it walks initializers
    one at a time without loading the full graph into memory. On a 17 GB
    decoder, the in-memory onnxconverter_common approach peaked over 30 GB
    and OOM-killed the parent process.
    """
    script = Path(__file__).parent / "cast_fp16.py"
