///
/// JLLMConfig.hpp
/// This file was generated by nitrogen. DO NOT MODIFY THIS FILE.
/// https://github.com/mrousavy/nitro
/// Copyright © Marc Rousavy @ Margelo
///

#pragma once

#include <fbjni/fbjni.h>
#include "LLMConfig.hpp"

#include "Backend.hpp"
#include "JBackend.hpp"
#include <optional>
#include <string>

namespace margelo::nitro::litertlm {

  using namespace facebook;

  /**
   * The C++ JNI bridge between the C++ struct "LLMConfig" and the the Kotlin data class "LLMConfig".
   */
  struct JLLMConfig final: public jni::JavaClass<JLLMConfig> {
  public:
    static constexpr auto kJavaDescriptor = "Lcom/margelo/nitro/dev/litert/litertlm/LLMConfig;";

  public:
    /**
     * Convert this Java/Kotlin-based struct to the C++ struct LLMConfig by copying all values to C++.
     */
    [[maybe_unused]]
    [[nodiscard]]
    LLMConfig toCpp() const {
      static const auto clazz = javaClassStatic();
      static const auto fieldSystemPrompt = clazz->getField<jni::JString>("systemPrompt");
      jni::local_ref<jni::JString> systemPrompt = this->getFieldValue(fieldSystemPrompt);
      static const auto fieldBackend = clazz->getField<JBackend>("backend");
      jni::local_ref<JBackend> backend = this->getFieldValue(fieldBackend);
      static const auto fieldMaxTokens = clazz->getField<jni::JDouble>("maxTokens");
      jni::local_ref<jni::JDouble> maxTokens = this->getFieldValue(fieldMaxTokens);
      static const auto fieldEngineMaxTokens = clazz->getField<jni::JDouble>("engineMaxTokens");
      jni::local_ref<jni::JDouble> engineMaxTokens = this->getFieldValue(fieldEngineMaxTokens);
      static const auto fieldOutputMaxTokens = clazz->getField<jni::JDouble>("outputMaxTokens");
      jni::local_ref<jni::JDouble> outputMaxTokens = this->getFieldValue(fieldOutputMaxTokens);
      static const auto fieldTemperature = clazz->getField<jni::JDouble>("temperature");
      jni::local_ref<jni::JDouble> temperature = this->getFieldValue(fieldTemperature);
      static const auto fieldTopK = clazz->getField<jni::JDouble>("topK");
      jni::local_ref<jni::JDouble> topK = this->getFieldValue(fieldTopK);
      static const auto fieldTopP = clazz->getField<jni::JDouble>("topP");
      jni::local_ref<jni::JDouble> topP = this->getFieldValue(fieldTopP);
      static const auto fieldValidate = clazz->getField<jni::JBoolean>("validate");
      jni::local_ref<jni::JBoolean> validate = this->getFieldValue(fieldValidate);
      return LLMConfig(
        systemPrompt != nullptr ? std::make_optional(systemPrompt->toStdString()) : std::nullopt,
        backend != nullptr ? std::make_optional(backend->toCpp()) : std::nullopt,
        maxTokens != nullptr ? std::make_optional(maxTokens->value()) : std::nullopt,
        engineMaxTokens != nullptr ? std::make_optional(engineMaxTokens->value()) : std::nullopt,
        outputMaxTokens != nullptr ? std::make_optional(outputMaxTokens->value()) : std::nullopt,
        temperature != nullptr ? std::make_optional(temperature->value()) : std::nullopt,
        topK != nullptr ? std::make_optional(topK->value()) : std::nullopt,
        topP != nullptr ? std::make_optional(topP->value()) : std::nullopt,
        validate != nullptr ? std::make_optional(static_cast<bool>(validate->value())) : std::nullopt
      );
    }

  public:
    /**
     * Create a Java/Kotlin-based struct by copying all values from the given C++ struct to Java.
     */
    [[maybe_unused]]
    static jni::local_ref<JLLMConfig::javaobject> fromCpp(const LLMConfig& value) {
      using JSignature = JLLMConfig(jni::alias_ref<jni::JString>, jni::alias_ref<JBackend>, jni::alias_ref<jni::JDouble>, jni::alias_ref<jni::JDouble>, jni::alias_ref<jni::JDouble>, jni::alias_ref<jni::JDouble>, jni::alias_ref<jni::JDouble>, jni::alias_ref<jni::JDouble>, jni::alias_ref<jni::JBoolean>);
      static const auto clazz = javaClassStatic();
      static const auto create = clazz->getStaticMethod<JSignature>("fromCpp");
      return create(
        clazz,
        value.systemPrompt.has_value() ? jni::make_jstring(value.systemPrompt.value()) : nullptr,
        value.backend.has_value() ? JBackend::fromCpp(value.backend.value()) : nullptr,
        value.maxTokens.has_value() ? jni::JDouble::valueOf(value.maxTokens.value()) : nullptr,
        value.engineMaxTokens.has_value() ? jni::JDouble::valueOf(value.engineMaxTokens.value()) : nullptr,
        value.outputMaxTokens.has_value() ? jni::JDouble::valueOf(value.outputMaxTokens.value()) : nullptr,
        value.temperature.has_value() ? jni::JDouble::valueOf(value.temperature.value()) : nullptr,
        value.topK.has_value() ? jni::JDouble::valueOf(value.topK.value()) : nullptr,
        value.topP.has_value() ? jni::JDouble::valueOf(value.topP.value()) : nullptr,
        value.validate.has_value() ? jni::JBoolean::valueOf(value.validate.value()) : nullptr
      );
    }
  };

} // namespace margelo::nitro::litertlm
