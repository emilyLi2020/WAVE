    ...
    return {
        ...
        loadModel: async (pathOrUrl, config, onDownloadProgress) => {
            ...
            if (pathOrUrl.startsWith("http://") || pathOrUrl.startsWith("https://")) {
                ...
                // Derive a cache filename from the URL. When the basename is a
                // generic name shared across model variants (e.g.
                // ".../litert-lm-v3/model.litertlm" and ".../mediapipe/model.litertlm"
                // both collapse to "model.litertlm"), prefix it with the parent path
                // segment so distinct variants don't collide on the same cache file
                // and serve a stale artifact. Unique basenames are left untouched.
                const segments = pathOrUrl.split("/").filter(Boolean);
                let fileName = segments.pop();
                if (!fileName) {
                    throw new Error(`Invalid model URL: ${pathOrUrl}`);
                }
                if (/^model\.(litertlm|task|bin|tflite)$/i.test(fileName) &&
                    segments.length > 0) {
