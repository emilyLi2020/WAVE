...
export interface LLMConfig {
     ...
     * @default 1024
     * @deprecated Sets BOTH the engine-wide token cache AND the per-call decode
     * cap to the same value, which is rarely correct: one value cannot be both
     * large enough to hold a long input prompt in the engine cache and small
     * enough to stay within a bundle's compiled decode-chunk shape. Use
     * {@link engineMaxTokens} + {@link outputMaxTokens} instead.
     */
    maxTokens?: number;
    /**
     * Engine-wide total token budget (input + output cache; the
     * `set_max_num_tokens` engine setting). Must be large enough to hold the
     * full input prompt with headroom for output.
     * @default 2048
     */
    engineMaxTokens?: number;
    /**
     * Per-call decode output cap (the `set_max_output_tokens` session setting).
     * Must NOT exceed the bundle's compiled decode-chunk size, or generation
     * fails with "failed to invoke the compiled model".
     * @default 1024
     */
