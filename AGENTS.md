...
- **Gemma 4 E2B-it (INT4) in the browser via `@huggingface/transformers` (transformers.js) + WebGPU.** Every word the patient sees during a session is generated by this model, fully on-device. No server-side LLM, no cloud LLM, no API key.
