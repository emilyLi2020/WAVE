...
- `models/train_wave_session_lora.py` is the active unified WAVE LoRA trainer. It consumes normalized input/output JSONL from `models/prepare_wave_session_dataset.py`, defaults to the Unsloth `unsloth/gemma-4-E2B-it` QLoRA path, applies the explicit `gemma-4` chat template, strips the leading `<bos>` from rendered SFT text, trains on assistant responses only, and evaluates base Gemma versus the LoRA. It writes token-length preflight reports, hard-fails truncation by default unless `--allow-truncation` is passed, saves periodic checkpoints with resume support, and defaults final eval to completion-only so generation eval can be run separately from a saved adapter. `models/train_phase_narration_lora.py` is the older phase-only trainer.
- Unified session LoRA data combines check-in 1-5, reflection, and phase narration sources. The current combined artifacts live at `models/datasets/lora-wave-session-expanded.jsonl` and `models/datasets/lora-wave-session-studio-sharegpt.jsonl`.
